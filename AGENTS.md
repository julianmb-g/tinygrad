...
* **Coverage Collapse due to Global Watchdogs**: A drastic dip in code coverage coupled with 100% unit test pass rates signifies a total suite deadlock. Overarching watchdogs (SIGKILL) on `pytest-xdist` runners erase stack traces and coverage artifacts. You MUST run `pytest -x -n 0` to organically catch the unmasked failures natively.
...
* **Planner Hallucination regarding Regex**: The planner may hallucinate the presence of regex like `--instrumentation_filter` in `harness.yaml` for submodules where it does not exist (e.g., `tinygrad`). Agents must verify the file contents before attempting to fix regex escapes.
* **Cross-Compilation Linker Evasion**: Using `-nostdlib` during cross-compilation tests (e.g., for E2E integration) WITHOUT explicitly invoking the authentic NPU linker script (`-T linker.ld`) completely bypasses DTCM and EXTMEM physical boundaries. All hardware boundary integration tests MUST invoke the correct linker script.
...
* **Test Pyramid Inversion**: Unmasking overarching E2E execution tests without establishing underlying functional unit/integration checks for local components (such as renderers and memory allocators) will result in stagnant code coverage and brittle architectures.
