...
* [FLAG: stale] **Missing Linker Placement for .noinit**
...
* [FLAG: stale] **Global IPC Exception Muzzling Ban:** Masking `OSError` ("cannot send") hides underlying async worker race conditions and shared memory starvation. Revert the `Connection.send` OS muzzling and write a proper Integration Test that verifies pytest-xdist thread teardowns synchronize and release resources without raising `OSError`.
* [FLAG: stale] **Stripped Register Bounds Risk:** Eradicating hardcoded CORALNPU register bounds without a VMM replacement allows the heuristic to fall through to generic DSP limits. Conflating register counts with DTCM SRAM byte capacities guarantees overflow natively.
