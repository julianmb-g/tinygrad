...
* **IPC Worker E2E Mocking Illusion (Testing Lesson)**: Exercising Python SharedMemory primitives with mocked tasks (e.g., numpy array doubling) fails to prove NPU execution. Tests must schedule authentic cross-compiled compute kernels to accurately measure shared memory teardown boundaries natively.
...
* **UOp Verification & Type Casting**: When decomposing integer operations (like `Ops.POW` via `xpow`) using floating-point transcendental functions (`log2`, `exp2`), the decomposition MUST explicitly cast and round its final output back to the original `base.dtype` to avoid polluting the graph with unintended `float` nodes. Failure to do so will violate `GroupOp.ALU` constraints and cause `UOp verification failed` crashes downstream.
* **Non-Existent Debug Methods**: Do not rely on non-existent methods like `dtypes.as_const` in debugging blocks or exception handlers (`spec.py`). Ensure exception handlers use valid property paths (`u.dtype.scalar().fmt` or just `type(u.arg)`).
* **Test Watchdog Teardown Failure**: When a test runner (like `pytest-xdist`) hits an overarching orchestrator timeout and loses its stack trace, you MUST run `pytest -x -n 0` to immediately halt on the first error and expose the root cause.
