...
class CoralNPURenderer(CStyleLanguage):
  ...
  def render(self, uops:list[UOp]) -> str:
    ...
    pools = [
      (0x00010000, 6144),  # Weights 0 (6KB)
      (0x00011800, 6144),  # Weights 1 (6KB)
      (0x00013000, 8192),  # Activations (8KB)
      (0x00015000, 8192)   # Accumulators (8KB)
    ]
  ...
  def render_kernel(self, function_name, kernel, bufs, uops, prefix=None) -> str:
    ...
    # Task 11.1: Ping Pong Tile transformation
    tiling_metrics = ping_pong_tile(uops)
    prefix.append(f"// PING_PONG_TILE: {tiling_metrics['ping_pong_tiles']} tiles generated")
    ...
    for name, (dtype, _) in bufs:
      ...
      # Allocate maximum VMM limit (28KB) per buffer to prevent OOM
      size_str = name.split("_")[-1] if "_" in name else "28672 / sizeof(" + c_type + ")"
