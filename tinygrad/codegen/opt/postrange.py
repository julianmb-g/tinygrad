  ...
  def apply_opt(self, opt:Opt, append_opt:bool=True):
    ...
    if opt.op in opt_to_at:
      ...
      if opt.op is OptOps.UPCAST:
        check((self.ren is not None and self.ren.target.device == "DSP") or (self.ren is not None and self.ren.target.device == "CORALNPU" and amt <= 28) or amt <= 16, "don't upcast more than 16/28")
