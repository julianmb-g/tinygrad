  ...
  def apply_opt(self, opt:Opt, append_opt:bool=True):
    ...
    if opt.op in opt_to_at:
      ...
      if opt.op is OptOps.UPCAST:
        check((self.ren is not None and self.ren.target.device == "DSP") or amt <= getattr(self.ren, "max_upcast", 16), f"don't upcast more than {getattr(self.ren, 'max_upcast', 16)}")
