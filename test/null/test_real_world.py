...
def helper_test(nm, gen, model, max_memory_allowed, max_kernels_allowed, expected_out=None, all_jitted=False):
  with Context(JIT=2):
    ...
    if all_jitted:
      assert kernels_used is not None and (
        (kernels_used > 0 and kernels_used == GlobalCounters.kernel_count) or \
        (kernels_used <= GlobalCounters.kernel_count and getattr(Device[Device.DEFAULT], "graph", None))
      ), f"only {kernels_used} out of {GlobalCounters.kernel_count} were jitted"
...
class TestRealWorld(unittest.TestCase):
  ...
  def test_stable_diffusion(self):
    ...
    def sd_gen():
      t1 = ((Tensor.arange(1*4*32*32)%10)*0.1).reshape(1, 4, 32, 32).cast(dtypes.float32)
      t2 = ((Tensor.arange(1*77*params["ctx_dim"])%10)*0.1).reshape(1, 77, params["ctx_dim"]).cast(dtypes.float32)
      return (t1, t2)
    helper_test("test_sd", sd_gen, test, 0.0105, 460, expected_out=[0.07505496, 0.18881409, -0.05983206])
  def test_unet_resblock(self):
    ...
    def test(t, t2):
      ...
      return t.realize()
    def unet_gen():
      t1 = ((Tensor.arange(4*16*8*8)%10)*0.1).reshape(4, 16, 8, 8).cast(dtypes.float32)
      t2 = ((Tensor.arange(1*24)%10)*0.1).reshape(1, 24).cast(dtypes.float32)
      return (t1, t2)
    helper_test("test_unet_resblock", unet_gen, test, 0.0002, 37, expected_out=[1.1154178, 2.4336586, -0.01451139])
  ...
  def test_train_mnist(self):
    ...
    with Tensor.train():
      ...
      helper_test("train_mnist", mnist_gen, train, 0.0105, 116, expected_out=1.8422501)
  ...
  def test_forward_cifar(self):
    ...
    with Tensor.train():
      ...
      helper_test("forward_cifar", cifar_gen, run, 0.0325, 30, expected_out=[0.3623826, 0.51155293, 0.2839802])
  ...
  def test_train_cifar(self):
    with Tensor.train():
      ...
      def train_cifar_gen():
        return (((Tensor.arange(BS*3*32*32)%10)*0.1).reshape(BS, 3, 32, 32).cast(dtypes.float32),)
      helper_test("train_cifar", train_cifar_gen, train, 0.110, 159, expected_out=0.35809016)
