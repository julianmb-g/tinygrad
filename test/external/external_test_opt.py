...
class TestInferenceMinKernels(unittest.TestCase):
  ...
  def test_convnext(self):
    ...
    img = ((Tensor.arange(1*3*224*224) % 10) * 0.1).reshape(1, 3, 224, 224)
    with CLCache(143):
      model(img).realize()
  def test_enet(self):
...
class TestOptBinOp(unittest.TestCase):
  def _test_no_binop_rerun(self, f1, f2=None, allowed=1):
    a = ((Tensor.arange(16*16) % 10) * 0.1).reshape(16, 16)
    b = ((Tensor.arange(16*16) % 10) * 0.1).reshape(16, 16)
...
class TestOpt(unittest.TestCase):
  def test_muladd(self):
    a,b,c = [((Tensor.arange(2*2) % 10) * 0.1).reshape(2,2).realize() for _ in range(3)]
  ...
  def test_permute_was_pushed(self):
    a = ((Tensor.arange(16*16*16) % 10) * 0.1).reshape(16, 16, 16)
    with CLCache(1):
      c = a.sum(2)
  ...
  def test_permute_was_pushed_through_contract_reshape(self):
    a = ((Tensor.arange(4*4*4*4*4) % 10) * 0.1).reshape(4, 4, 4, 4, 4)
    with CLCache(1):
      c = a.sum(-1)
