...
class TestSpecific(unittest.TestCase):
  ...
  def test_1x1_6_24(self):
    x = ((Tensor.arange(1*24*4*32*64) % 10) * 0.1).reshape(1,   24*4, 32, 64)
    w = ((Tensor.arange(6*4*24*4*1*1) % 10) * 0.1).reshape(6*4, 24*4, 1,  1)
  ...
  def test_vec_mul(self):
    ...
    w = ((Tensor.arange(2048*512) % 10) * 0.1).reshape(2048, 512)
  ...
  def test_big_vec_mul(self):
    ...
    x = ((Tensor.arange(4096) % 10) * 0.1).reshape(4096).realize()
    w = ((Tensor.arange(4096*4096) % 10) * 0.1).reshape(4096, 4096).cast(dtypes.float16).realize()
    (x @ w.T).realize()
  ...
  def test_1x1_28_28(self):
    x = ((Tensor.arange(1*256*28*28) % 10) * 0.1).reshape(1,   256, 28, 28)
    w = ((Tensor.arange(256*256*1*1) % 10) * 0.1).reshape(256, 256, 1,  1)
  ...
  def test_3x3_28_28_stride_2(self):
    x = ((Tensor.arange(1*288*36*36) % 10) * 0.1).reshape(1,   288, 36, 36)
    w = ((Tensor.arange(384*288*3*3) % 10) * 0.1).reshape(384, 288, 3,  3)
