...
class TestFP8Linear(unittest.TestCase):
  ...
  def _test_forward(self, shape, in_features, out_features):
    ...
    weight = ((Tensor.arange(out_features*in_features) % 10) * 0.1).reshape(out_features, in_features).cast(dtypes.float32)  * 0.2
    bias = ((Tensor.arange(out_features) % 10) * 0.1).reshape(out_features).cast(dtypes.float32)  * 0.2
    fp8_layer.weight.assign(weight)
    normal_layer.weight.assign(weight)
    fp8_layer.bias.assign(bias)
    normal_layer.bias.assign(bias)
    x = ((Tensor.arange(math.prod(shape)) % 10) * 0.1).reshape(*shape).cast(dtypes.float32)  * 0.2
  ...
  def _test_backward(self, shape, in_features, out_features):
    ...
    x_fp8 = ((Tensor.arange(math.prod(shape)) % 10) * 0.1).reshape(*shape).cast(dtypes.float32).requires_grad_(True)  * 0.2
  ...
  def test_filter(self):
    ...
    model = Model()
    x = ((Tensor.arange(16*32) % 10) * 0.1).reshape(16, 32)
  ...
  def test_multi_gpu(self):
    ...
    weight = ((Tensor.arange(out_dim*in_dim) % 10) * 0.1).reshape(out_dim, in_dim).cast(dtypes.float32) * 0.2
    bias = ((Tensor.arange(out_dim) % 10) * 0.1).reshape(out_dim).cast(dtypes.float32) * 0.2
    fp8_layer.weight.assign(weight)
    fp8_layer.bias.assign(bias)
    normal_layer.weight.assign(weight)
    normal_layer.bias.assign(bias)
